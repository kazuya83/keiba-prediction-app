...
describe("RegisterForm", () => {
  ...
  it("パスワードが一致しない場合にエラーが表示される", async () => {
    const user = userEvent.setup();
    render(<RegisterForm />, { wrapper: createWrapper() });
  ...
  it("パスワード強度が表示される", async () => {
    ...
    await waitFor(() => {
      // パスワード強度インジケーターが表示されることを確認
      const strengthIndicator = screen.queryByText(/弱い|普通|強い/);
      expect(strengthIndicator).toBeInTheDocument();
    });
  });

  it("登録成功時にトークンが設定され、ダッシュボードにリダイレクトされる", async () => {
    const user = userEvent.setup();
    const mockToken = {
